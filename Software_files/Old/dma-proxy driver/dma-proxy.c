...
// ************************************************** File Operation Helper Function **************************************************
...
{
	/* These are the steps to start a DMA transfer
     * Step 1: Initialize DMA control registers
     * Step 2: Obtain a descriptor using the device_prep_slave function
     * Step 3: Attach the completion function to the descriptor
     * Step 4: Initialize the completion data structure
     * Step 5: Submit the transfer request to the queue and obtain its cookie using dmaengine_submit
     * Step 6: Start DMA transfer using dma_async_issue_pending
     */ 
...
/* 
 * file_operation Data Structure
 * Defines the file operations
 * Will be used to connect driver to Kernel
 * These include owner, open, write, read, release and etc
 */
...
{
	/*
     * The following steps are taken to initialize a character device
     * Step 1: Dynamically allocate a major and minor number to the device using
     * the alloc_chrdev_region function
     * Step 2: Initialize the cdev structure and attach the fops structure to it
     * using the cdev_init function and add it to the Kernel using cdev_add
     * Step 3: If a class has not been created before, create a class using class_create
     * Step 4: Create the device node using device_create. This addes the device to /dev.
     */
...
static int create_channel(struct platform_device *pdev, struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
     ...
     * Step 1: A slave DMA channel is requested using dma_request_slave_channel. This function takes
     * care of creating the capabilities mask for a slave device. This step fills the channel_p and
     * dma_device_p of the channel structure.
     * Step 2: Initialize the character device using the cdevice_init function
     * Step 3: Set the direction field of DMA channel
     * Step 4: Fill the interface field of channel structure by allocating managed unchached memory
     * using dmam_alloc_coherent
     */ 

	int rc;

	// Step 1: A slave DMA channel is requested using dma_request_slave_channel
