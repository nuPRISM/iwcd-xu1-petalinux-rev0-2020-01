...
int main(int argc, char *argv[])
{
	...
	// Load dma-proxy module if it is not loaded
	printf("\nChecking if dma-proxy is loaded... \n");

	module_fd = popen("lsmod | grep dma-proxy", "r");
	if(fread(buf, 1, sizeof (buf), module_fd) > 0) {
		printf("dma-proxy is loaded! \n");
	}
	else {
		printf("dma-proxy is not loaded! \n");
		printf("Loading dma-proxy... \n");
		system("modprobe dma-proxy");
	}
	...
	for (counter = 0; counter < atoi(argv[1]); counter++) {
		 ...
		 */
		printf("flushing rx buffer and requesting receive transfer...\n");
		for (i = 0; i < test_size; i++)
		...
		/* Verify the data recieved matches what was sent (tx is looped back to tx)
		 */
		for (i = 0; i < test_size; i++)
